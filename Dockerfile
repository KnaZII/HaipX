# Build docker container: docker build -t haipx .
# Build project: docker run --rm -it -v$(pwd):/project haipx bash -c "cmake -DCMAKE_BUILD_TYPE=Release -DCMAKE_INSTALL_RPATH_USE_LINK_PATH=TRUE -Bbuild && cmake --build build"
# Run project in docker: docker run --rm -it -v$(pwd):/project -v/tmp/.X11-unix:/tmp/.X11-unix -v${XAUTHORITY}:/home/user/.Xauthority:ro -eDISPLAY --network=host haipx ./build/HaipX

FROM debian:bookworm-slim
LABEL Description="Docker container for building HaipX for Linux"
