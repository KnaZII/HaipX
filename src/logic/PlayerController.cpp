...
void CameraControl::updateMouse(PlayerInput& input, int windowHeight, float delta, Input* inputSystem, bool uiBlocking) {
...
void PlayerController::postUpdate(
    float delta, int windowHeight, const Input* input, bool pause, bool uiBlocking
) {
    if (!pause) {
    ...
    if (!pause && input) {
        camControl.updateMouse(this->input, windowHeight, delta, const_cast<Input*>(input), uiBlocking);
    }
...
void PlayerController::updateKeyboard(const Input& inputEvents) {
    ...
    // Command system
    updateCommandInput(inputEvents);
}
...
void PlayerController::updateInteraction(const Input& inputEvents, float delta) {
    ...
    if (vox == nullptr) {
        // Останавливаем разрушение если не смотрим на блок
        if (isBreakingBlock) {
            stopBreakingBlock();
        }
    ...
    // Обработка разрушения блоков с помощью предметов
    ...
    // Система постепенного разрушения блоков
    if (lclick && target.breakable) {
        // Проверяем, смотрим ли мы на тот же блок
        if (!isBreakingBlock || breakingX != iend.x || breakingY != iend.y || breakingZ != iend.z) {
            // Начинаем разрушение нового блока
            startBreakingBlock(iend.x, iend.y, iend.z);
        }
        
        // Продолжаем разрушение
        continueBreakingBlock(delta);
    } else {
        // Останавливаем разрушение если не нажимаем левую кнопку
        if (isBreakingBlock) {
            stopBreakingBlock();
        }
        
        // Оригинальная логика мгновенного разрушения для креативного режима
        if (lclick && player.isInstantDestruction() && target.breakable) {
            scripting::on_block_breaking(&player, target, iend);
            blocksController.breakBlock(&player, target, iend.x, iend.y, iend.z);
        }
    }
...
void PlayerController::toggleCommandMode() {
...
void PlayerController::updateCommandInput(const Input& inputEvents) {
    if (!commandMode) {
        // Проверяем нажатие TAB для входа в режим команд
        if (inputEvents.jpressed(Keycode::TAB)) {
            toggleCommandMode();
        }
        return;
    }
    
    // Обработка ввода в режиме команд
    for (int key = static_cast<int>(Keycode::A); key <= static_cast<int>(Keycode::Z); key++) {
        if (inputEvents.jpressed(static_cast<Keycode>(key))) {
            char c = 'a' + (key - static_cast<int>(Keycode::A));
            commandBuffer += c;
        }
    }
    
    // Пробел
    if (inputEvents.jpressed(Keycode::SPACE)) {
        commandBuffer += ' ';
    }
    
    // Backspace
    if (inputEvents.jpressed(Keycode::BACKSPACE)) {
    ...
    if (inputEvents.jpressed(Keycode::ENTER)) {
        if (!commandBuffer.empty()) {
    ...
    // Escape для отмены
    if (inputEvents.jpressed(Keycode::ESCAPE)) {
        toggleCommandMode();
    }
}

void PlayerController::processCommand(const std::string& command) {
    std::string cmd = command;
    std::transform(cmd.begin(), cmd.end(), cmd.begin(), ::tolower);
    
    // Функция для вывода сообщений через Lua
    auto logMessage = [](const std::string& message) {
        auto L = lua::get_main_state();
        if (L && lua::getglobal(L, "console")) {
            if (lua::getfield(L, "log", -1)) {
                lua::pushstring(L, message);
                lua::call(L, 1, 0);
            }
            lua::pop(L); // pop console
        }
    };
    ...
    if (cmd == "commands" || cmd == "help") {
        ...
        logMessage("commands - показать эту справку");
        logMessage("survival - переключиться в режим выживания");
        logMessage("creative - переключиться в креативный режим");
        logMessage("gamemode 0 - режим выживания");
        logMessage("gamemode 1 - креативный режим");
        logMessage("tp <x> <y> <z> - телепортация");
        logMessage("give <item> <count> - дать предмет");
        logMessage("time <day/night> - установить время");
        logMessage("weather <clear/rain/snow> - установить погоду");
        logMessage("========================");
    } else if (cmd == "survival") {
        player.setSurvivalMode(true);
        player.setFlight(false);
        player.setNoclip(false);
        player.setInfiniteItems(false);
        player.setInstantDestruction(false);
        logMessage("Режим выживания включен");
    } else if (cmd == "creative") {
        player.setSurvivalMode(false);
        player.setFlight(true);
        player.setNoclip(false);
        player.setInfiniteItems(true);
        player.setInstantDestruction(true);
        logMessage("Креативный режим включен");
    } else if (cmd == "gamemode 0" || cmd == "gamemode survival") {
        player.setSurvivalMode(true);
        player.setFlight(false);
        player.setNoclip(false);
        player.setInfiniteItems(false);
        player.setInstantDestruction(false);
        logMessage("Режим выживания включен");
    } else if (cmd == "gamemode 1" || cmd == "gamemode creative") {
        player.setSurvivalMode(false);
        player.setFlight(true);
        player.setNoclip(false);
        player.setInfiniteItems(true);
        player.setInstantDestruction(true);
        logMessage("Креативный режим включен");
    } else if (cmd.substr(0, 2) == "tp ") {
        // Команда телепортации: tp <x> <y> <z>
        std::string args = cmd.substr(3);
        std::istringstream iss(args);
        float x, y, z;
        if (iss >> x >> y >> z) {
            player.teleport(glm::vec3(x, y, z));
            logMessage("Телепортация в позицию (" + std::to_string(x) + ", " + std::to_string(y) + ", " + std::to_string(z) + ")");
        } else {
            logMessage("Ошибка: используйте 'tp <x> <y> <z>'");
        }
    } else {
        logMessage("Неизвестная команда: " + command);
        logMessage("Введите 'commands' для списка команд");
    }
}

// Block breaking system implementation
void PlayerController::startBreakingBlock(int x, int y, int z) {
    // Останавливаем предыдущее разрушение
    if (isBreakingBlock) {
        stopBreakingBlock();
    }
    
    // Начинаем новое разрушение
    isBreakingBlock = true;
    breakingX = x;
    breakingY = y;
    breakingZ = z;
    breakingProgress = 0.0f;
    breakingTime = getBlockBreakingTime(x, y, z);
    
    // Добавляем трещины на блок (пока отключено, так как нет API)
    breakingCrackId = 1; // Временный ID
}

void PlayerController::continueBreakingBlock(float delta) {
    if (!isBreakingBlock) {
        return;
    }
    
    // Увеличиваем прогресс разрушения
    breakingProgress += delta / breakingTime;
    
    // Обновляем трещины в зависимости от прогресса (пока отключено)
    // TODO: Добавить визуальные трещины на блоке
    
    // Проверяем, завершено ли разрушение
    if (breakingProgress >= 1.0f) {
        // Разрушаем блок
        auto indices = level.content.getIndices();
        auto vox = player.chunks->get(breakingX, breakingY, breakingZ);
        if (vox) {
            auto& blockDef = indices->blocks.require(vox->id);
            scripting::on_block_breaking(&player, blockDef, glm::ivec3(breakingX, breakingY, breakingZ));
            blocksController.breakBlock(&player, blockDef, breakingX, breakingY, breakingZ);
        }
...
void PlayerController::stopBreakingBlock() {
    if (!isBreakingBlock) {
        return;
    }
    
    // Убираем трещины (пока отключено)
    // TODO: Убрать визуальные трещины с блока
...
float PlayerController::getBlockBreakingTime(int x, int y, int z) {
    auto vox = player.chunks->get(x, y, z);
    if (!vox) {
        return 1.0f;
    }
    ...
    // Получаем прочность блока из его определения
    float durability = 3.0f; // Значение по умолчанию
    
    // Определяем время разрушения по имени блока
    std::string blockName = blockDef.name;
    
    // Мгновенное разрушение
    if (blockName.find("grass") != std::string::npos || 
        blockName.find("flower") != std::string::npos ||
        blockName.find("torch") != std::string::npos) {
        return 0.0f;
    }
    
    // Быстрое разрушение (1.5 сек)
    if (blockName.find("dirt") != std::string::npos ||
    ...
    if (blockName.find("wood") != std::string::npos ||
        ...
        blockName.find("brick") != std::string::npos ||
        blockName.find("glass") != std::string::npos ||
        blockName.find("ice") != std::string::npos) {
        return 3.0f;
    }
    
    // Медленное разрушение (8.0+ сек)
    if (blockName.find("stone") != std::string::npos) {
        return 8.0f;
    }
    
    if (blockName.find("metal") != std::string::npos) {
        return 10.0f;
    }
    
    if (blockName.find("bazalt") != std::string::npos) {
        return 12.0f;
    }
