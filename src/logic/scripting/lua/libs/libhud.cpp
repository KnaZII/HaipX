...
static int l_create_image(lua::State* L) {
    auto texture = lua::require_string(L, 1);
    auto width = lua::tointeger(L, 2);
    auto height = lua::tointeger(L, 3);
    
    // Создаем простой элемент изображения
    auto image = std::make_shared<gui::Container>(engine->getGUI(), glm::vec2(width, height));
    
    // Возвращаем объект изображения
    lua_pushlightuserdata(L, image.get());
    return 1;
}

static int l_get_element(lua::State* L) {
    auto id = lua::require_string(L, 1);
    
    // Ищем элемент по ID
    auto element = engine->getGUI().get(id);
    if (element) {
        lua_pushlightuserdata(L, element.get());
        return 1;
    }
...
const luaL_Reg hudlib[] = {
    ...
    {"create_image", wrap_hud<l_create_image>},
    {"get_element", wrap_hud<l_get_element>},
    {NULL, NULL}
};
